...
def _get_severity(flag_type):
    if flag_type == LOW_YIELD_FLAG_TYPE:
        return "Medium"
    return "High"
...
def _sync_anomalies(db: Session, property_obj, rules):
    ...
    for anomaly in existing:
        ...
        if rule is None:
            ...
            continue
        description = rule
        severity = _get_severity(anomaly.flag_type)
    ...
    for flag_type, description in rules.items():
        anomaly = Anomaly(
            ...
            severity=_get_severity(flag_type),
        )
        db.add(anomaly)
        active.append(anomaly)
...
def get_saved_anomaly(db: Session, property_id):
    property_obj = db.query(Property).filter(Property.id == property_id).first()
    if not property_obj:
        raise ValueError("Property not found")
    ...
    audit = db.query(Audit).filter(Audit.property_id == property_id).first()
    _, price_per_square_meter, peer_average, comparables = _build_rules(db, property_obj, listing_obj, audit)
    active = db.query(Anomaly).filter(Anomaly.property_id == property_obj.id).all()
    anomalies = _serialize(active)
    
    return _build_response(property_obj, audit, anomalies, False, price_per_square_meter, peer_average, comparables)
